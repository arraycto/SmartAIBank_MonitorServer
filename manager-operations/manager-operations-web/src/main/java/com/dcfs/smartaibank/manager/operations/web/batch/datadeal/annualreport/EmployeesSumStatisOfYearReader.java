...
@Slf4j
@StepScope
public class EmployeesSumStatisOfYearReader extends AbstractTasklet {
	@Autowired
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		log.info("柜员视角汇总数据-->年统计，传入参数：[dateType,yearStr]=[{},{}]",
			Constants.YEAR, year);
		try {
			operationDataStatsDao.userGather(Constants.YEAR, year);
		} catch (Exception e) {
			log.error("柜员视角-汇总数据年统计发生异常", e);
		}
