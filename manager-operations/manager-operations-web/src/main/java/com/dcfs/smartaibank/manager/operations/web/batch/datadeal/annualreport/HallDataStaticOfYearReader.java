...
@Slf4j
@StepScope
public class HallDataStaticOfYearReader extends AbstractTasklet {
	@Autowired
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		log.info("厅堂数据按照机构统计-->年统计，传入参数：[dateType,year]=[{},{}]",
			Constants.YEAR, year);
		try {
			operationDataStatsDao.hallMarketStats(Constants.YEAR, year, "");
		} catch (Exception e) {
			log.error("厅堂机构数据年统计发生异常", e);
		}
