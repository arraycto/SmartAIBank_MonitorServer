...
@Slf4j
@StepScope
public class BusEfficStatisByChannelAndTradeOfDayReader extends AbstractTasklet {
	@Autowired
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		log.info("业务效率按照交易类型统计-->日统计，传入参数：[date,dateType,month]=[{},{},{}]",
			date, Constants.DAY, month);
		try {
			operationDataStatsDao.busEfficTran(date, Constants.DAY, month);

		} catch (Exception e) {
			log.error("业务效率按照交易类型统计-->日统计，发生异常", e);
		}
