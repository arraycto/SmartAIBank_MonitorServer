...
@Slf4j
@StepScope
public class EmployeesSumStatisOfDayReader extends AbstractTasklet {
	@Autowired
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		log.info("柜员视角汇总数据-->日统计，传入参数：[dateType,date]=[{},{}]",
			Constants.DAY, date);
		try {
			operationDataStatsDao.userGather(Constants.DAY, date);
		} catch (Exception e) {
			log.error("柜员视角-汇总数据日统计发生异常", e);
		}
		return null;
	}
