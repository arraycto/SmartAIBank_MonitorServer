...
@Slf4j
@StepScope
public class HallDataUserStaticOfDayReader extends AbstractTasklet {
	@Autowired
	private OperationDataStatsDao operationDataStatsDao;

	public HallDataUserStaticOfDayReader() {
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		log.info("厅堂用户数据-->日统计，传入参数：[dateType,date,month]=[{},{},{}]",
			Constants.DAY, date, month);
		try {
			operationDataStatsDao.hallMarketUserStats(Constants.DAY, date, month);
		} catch (Exception e) {
			log.error("厅堂用户数据日统计发生异常", e);
		}
