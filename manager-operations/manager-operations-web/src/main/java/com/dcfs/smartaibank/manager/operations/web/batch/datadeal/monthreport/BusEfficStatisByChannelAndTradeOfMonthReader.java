...
@Slf4j
@StepScope
public class BusEfficStatisByChannelAndTradeOfMonthReader extends AbstractTasklet {
	@Autowired
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		log.info("业务效率按照交易类型统计-->月统计，传入参数：[monthStr,dateType,quarter]=[{},{},{}]",
			monthStr, Constants.MONTH, quarter);
		try {
			operationDataStatsDao.busEfficTran(monthStr, Constants.MONTH, quarter);
		} catch (Exception e) {
			log.error("业务效率按照交易类型统计-->月统计，发生异常", e);
		}
