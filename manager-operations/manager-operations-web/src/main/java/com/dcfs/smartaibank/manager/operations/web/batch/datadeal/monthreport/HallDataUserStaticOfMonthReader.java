...
@Slf4j
@StepScope
public class HallDataUserStaticOfMonthReader extends AbstractTasklet {
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		log.info("厅堂用户数据-->月统计，传入参数：[dateType,monthStr,quarter]=[{},{},{}]",
			Constant.MONTH, monthStr, quarter);
		try {
			operationDataStatsDao.hallMarketUserStats(Constant.MONTH, monthStr, quarter);
		} catch (Exception e) {
			log.error("厅堂用户数据月统计发生异常", e);
		}
