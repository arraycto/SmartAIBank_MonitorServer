...
@Slf4j
@StepScope
public class BusEfficStatisByChannelAndTradeOfQuarterReader extends AbstractTasklet {
	@Autowired
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		log.info("业务效率按照交易类型统计-->季度统计，传入参数：[quarterStr,dateType,year]=[{},{},{}]",
			quarterStr, Constant.QUARTER, year);
		try {
			operationDataStatsDao.busEfficTran(quarterStr, Constant.QUARTER, year);
		} catch (Exception e) {
			log.error("业务效率按照交易类型统计-->季度统计，发生异常", e);
		}
