...
@Slf4j
@StepScope
public class OrganizationSumStatisOfQuarterReader extends AbstractTasklet {
	@Autowired
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		log.info("机构汇总数据-->季统计，传入参数：[dateType,quarterStr]=[{},{}]",
			Constant.QUARTER, quarterStr);
		try {
			operationDataStatsDao.orgGather(Constant.QUARTER, quarterStr);
		} catch (Exception e) {
			log.error("机构汇总数据季统计发生异常", e);
		}
