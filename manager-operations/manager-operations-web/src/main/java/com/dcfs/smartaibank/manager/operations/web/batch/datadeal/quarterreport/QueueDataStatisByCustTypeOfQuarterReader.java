...
@Slf4j
@StepScope
public class QueueDataStatisByCustTypeOfQuarterReader extends AbstractTasklet {
	@Autowired
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		log.info("排队客户类型数据-->季统计，传入参数：[dateType,quarterStr]=[{},{}]",
			Constant.QUARTER, quarterStr);
		try {
			operationDataStatsDao.queueByCust(Constant.QUARTER, quarterStr);
		} catch (Exception e) {
			log.error("排队-客户类型数据季统计发生异常", e);
		}
